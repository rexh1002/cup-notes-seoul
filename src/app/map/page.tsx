              ...
              try {
                ...
                if (!mapInstance) {
                  ...
                  if (retryCount < maxRetries) {
                    ...
                    console.error('[모바일] 지도 인스턴스 준비 시간 초과');
                  }
                  return;
                }

                const firstCafe = data.cafes[0];
                console.log('[모바일] 첫 번째 카페 위치로 이동:', firstCafe.name, firstCafe.latitude, firstCafe.longitude);
                
                const newCenter = new window.naver.maps.LatLng(firstCafe.latitude, firstCafe.longitude);
                mapInstance.setCenter(newCenter);
                console.log('[모바일] 지도 중심 이동 완료');
              } catch (error) {
                console.error('[모바일] 지도 이동 중 에러 발생:', error);
              }
            };
